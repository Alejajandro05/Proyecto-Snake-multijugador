...
export class LocalGame extends Phaser.Scene {
    ...
    create() {
        ...
        // HU-034: suscribir el evento de consumo de comida.
        // • Si el tipo es distinto de apple se dispara el efecto visual (partículas + texto flotante).
        // • La manzana sólo reproduce el sonido ya existente; el renderer la ignora internamente.
        this.engine.events.on('playerEatFood', ({ playerId, food, foodItem }) => {
            const hud = playerId === P1_ID ? this.hudJ1Effect : this.hudJ2Effect;
            this.showPlayerEffect(food, hud, playerId);

            // Disparar efecto visual para alimentos especiales (apple se descarta dentro del layer)
            if (foodItem) {
                this.boardRenderer.triggerFoodEffect(foodItem.type, foodItem.x, foodItem.y);
            }
        });
        ...
        this.events.on('shutdown', () => {
            ...
            // HU-034: limpiar el layer de efectos visuales al salir
            this.boardRenderer.destroyEffects();
        });
    ...
    showPlayerEffect(food, hudElement, playerId) {
        ...
        if (this.effectTimeouts[playerId]) clearTimeout(this.effectTimeouts[playerId]);
